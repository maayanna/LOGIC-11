...
class Formula:
    ...
    def variables(self) -> Set[str]:
        ...
        if is_unary(self.root):
            vars.update(self.first.variables())

        if is_binary(self.root):
            vars.update(self.first.variables())
            vars.update(self.second.variables())

        if is_quantifier(self.root):
            vars.add(self.variable)
            vars.update(self.predicate.variables())
    ...
    def free_variables(self) -> Set[str]:
        ...
        if is_variable(self.root):
            free.update(self.root)

        if is_equality(self.root) or is_relation(self.root):

            for arg in self.arguments:
                free.update(arg.variables())

        elif is_unary(self.root):
            free.update(self.first.free_variables())

        elif is_binary(self.root):
            free.update(self.first.free_variables(), self.second.free_variables())

        elif is_quantifier(self.root):
            free.update(self.predicate.free_variables().difference({self.variable}))
    ...
    def substitute(self, substitution_map: Mapping[str, Term],
        ...
        if is_constant(self.root) or is_variable(self.root) or is_function(self.root):
            ...
            # CHANGE FOR EX11
            old_sub = substitution_map
            substitution_map = dict()

            for var in old_sub.keys():
                if var != self.variable:
                    substitution_map[var] = old_sub[var]
            ...
            forbids = set(forbidden_variables)
            forbids = forbids.union({self.variable})
            predicate = self.predicate.substitute(substitution_map, forbids)
