...
class Schema:
    ...
    def _instantiate_helper(formula: Formula,
        ...
        if is_equality(formula.root):
            ...
            first = Schema._instantiate_helper(new_formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            second = Schema._instantiate_helper(new_formula.second, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables)
            return Formula(new_formula.root, first, second)

        elif is_unary(new_formula.root):
            return Formula(new_formula.root, Schema._instantiate_helper(new_formula.first, constants_and_variables_instantiation_map, relations_instantiation_map, bound_variables))

        elif is_quantifier(new_formula.root):
            var = new_formula.variable
            if new_formula.variable in constants_and_variables_instantiation_map:
                var = constants_and_variables_instantiation_map[new_formula.variable]
            new_forbids = set(bound_variables)
            new_forbids.add(str(var))
            prds = Schema._instantiate_helper(new_formula.predicate, constants_and_variables_instantiation_map, relations_instantiation_map, new_forbids)
            return  Formula(new_formula.root, str(var), prds)

        elif is_relation(new_formula.root):
            ...
            if len(formula.arguments) == 0:
                if len(relations_instantiation_map[formula.root].free_variables().intersection(bound_variables)) > 0:
                    raise Schema.BoundVariableError(
                        (relations_instantiation_map[formula.root].variables().intersection(bound_variables))[-1],
                        formula.root)
                return relations_instantiation_map[formula.root]

            else:
                new_sub_map = {'_': formula.arguments[0].substitute(constants_and_variables_instantiation_map)}
                form = relations_instantiation_map[formula.root]
                if len(form.free_variables().intersection(bound_variables)) > 0:
                    raise Schema.BoundVariableError((form.free_variables().intersection(bound_variables))[-1],
                                                    formula.root)
                return form.substitute(new_sub_map)
        ...
        try:
            ...
            for key in instantiation_map:
                ...
                if is_constant(key) or is_variable(key):
                    consts_var[key] = Term.parse(str(instantiation_map[key]))
                if is_relation(key):
                    rel[key] = instantiation_map[key]
            return Schema._instantiate_helper(self.formula, consts_var, rel , set())
...
class Proof:
    ...
    class MPLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            left = lines[self.antecedent_line_number].formula.__repr__()
            right = self.formula.__repr__()
            str_check = "(" + left + "->" + right + ")"

            # print(lines[self.conditional_line_number].formula.__repr__())

            return str_check == lines[self.conditional_line_number].formula.__repr__()
    ...
    class UGLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            if self.formula.root != "A" or self.predicate_line_number >= line_number:
                return False

            pred = self.formula.predicate
            return pred == lines[self.predicate_line_number].formula
    ...
    class TautologyLine:
        ...
        def is_valid(self, assumptions: AbstractSet[Schema],
            ...
            old_form = self.formula.propositional_skeleton()[0]
            return is_propositional_tautology(old_form)
...
def axiom_specialization_map_to_schema_instantiation_map(
    ...
    for key, val in propositional_specialization_map.items():
        dic_return[key.upper()] = Formula.from_propositional_skeleton(val, substitution_map)
...
def prove_from_skeleton_proof(formula: Formula,
    ...
    for line in skeleton_proof.lines:
        ...
        if PropositionalProof.Line.is_assumption(line):
            form = Formula.from_propositional_skeleton(line.formula, substitution_map)
            schema_new = Schema(form, set())
            lines.append(Proof.AssumptionLine(form, schema_new, dict()))

        elif line.rule == MP:
            form = Formula.from_propositional_skeleton(line.formula, substitution_map)
            lines.append(Proof.MPLine(form, line.assumptions[0], line.assumptions[1]))

        else: #different rule
            map = PropositionalInferenceRule.formula_specialization_map(line.rule.conclusion, line.formula)
            inst_map = axiom_specialization_map_to_schema_instantiation_map(map, substitution_map)
            form = Formula.from_propositional_skeleton(line.formula, substitution_map)

            lines.append(Proof.AssumptionLine(form, PROPOSITIONAL_AXIOM_TO_SCHEMA[line.rule],inst_map))
...
def prove_tautology(tautology: Formula) -> Proof:
    ...
    skeleton, map = tautology.propositional_skeleton()
    proof = prove_propositional_tautology(skeleton)
    return prove_from_skeleton_proof(tautology, proof, map)
